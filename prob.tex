...
\begin{tikzpicture}[scale=.5]
    % Fundo preto ajustado (expandido)
    \fill[black] (-4,-2.5) rectangle (7.5,4);
    ...
    % Rótulos P(E|H)
    \node[white] at (-1.2,0.7) {$P(E|H) \left\{\begin{array}{c} \\ \\ \end{array}\right.$};

    % Rótulos P(H)
    \node[white] at (3,-0.8) {$\underbrace{\hspace{3cm}}_{P(H)}$};

    % Texto E e H em cores diferentes
   % \node[white,scale=1.3] at (-1.7,2.7) {$P($\textcolor{lightblue}{$E$}$|$\textcolor{yellow}{$H$}$)$};

    % Texto H em cor diferente no termo P(H)
    %\node[white,scale=1.3] at (3,-1.5) {$P($\textcolor{yellow}{$H$}$)$};
\end{tikzpicture}
